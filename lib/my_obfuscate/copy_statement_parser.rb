      ...
      input_io.each do |line|
        if parse_insert_statement(line)
          ...
          if !config[current_table_name]
            $stderr.puts "Deprecated: #{current_table_name} was not specified in the config.  A future release will cause this to be an error.  Please specify the table definition or set it to :keep." unless obfuscator.ignore_depricated_warnings?
          end
